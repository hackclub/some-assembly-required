...
* To run on x86-64 processor, AT&T syntax:
* gcc uppercaser-linux.s -o uppercaser-linux && ./uppercaser-linux "String to convert"
*/
...
/*
* In an x86-64 processor (System V) at the beginning of main:
...
main:
  ...
  *
  * In the AT&T syntax instruction parameters are all reversed in comparison to the Intel syntax.
  * Eg. mov takes source as the first argument and destination as the second one (mov src, dst).
  */
  mov %rdi, %r12 /* argc */
  mov %rsi, %r13 /* argv[][] */
  ...
  * Be aware that in the AT&T syntax cmp's instruction parameters are reversed too!
  */
  cmp $1, %rdi
  je .exit /* jump to .exit if rdi == 1 */
...
.printNumberOfArgs:
  ...
  */
  ...
  /*
  * write() syscall will use this to determine how many bytes to print. Since we're only printing 1 character, we only want
...
.printNewline:
  ...
  * rsi, so that write() will print it later (see the explanation in .printNumberOfArgs for more info on write() Linux syscall)
  */
  mov %rsp, %rsi

  /*
  * write() will use this to determine how many bytes to print. Since we're only printing 1 character, we only want
  * to print 1 byte.
  */
  mov $1, %rdx
...
.printAllArgs:
  ...
  */
  ...
  .printArgsLoopStart:
    ...
    */
    jl .printArgsLoopStart /* jump to .printArgsLoopStart if rax < rdi */
...
.printArg:
  ...
  *
  * Finally, the -q postfix is necessary because, when we're following addresses into memory, we need to tell the
  ...
  * that the write() syscall can write it when we call .print below.
  *
...
.iterate:
  ...
  */
  ...
  .iterateLoop:
    ...
    * Counterintuitively, in the AT&T syntax when we want to dereference a memory address that is
    * a sum of values in two registers we write it as (%reg1, %reg2)
    * (see https://en.wikibooks.org/wiki/X86_Assembly/GNU_assembly_syntax). The Intel syntax equivalent is [reg1 + reg2].
    ...
    * one byte from memory, so we postfix the cmp instruction with "b".
    *
    * What's cmpb $0, (%rsi, %rax) ?
    * The cmp instruction compares two values, and records which one is greater (or if they're equal) in a special
    * register called the EFLAGS register. Don't worry about the details of EFLAGS for now; just remember that, after
    * running the cmp instruction, the result of the comparison is saved into EFLAGS.
...
.print:
  /*
  * Specify that we want syscall to write some data.
  * rax is used to tell the OS what syscall we want to call.
  * 1 is used to say we want to use the Linux write() syscall.
  *
  ...
  */
  ...
  /*
  * Set argument 1 (rdi) to stdout. This tells write() syscall that we want to write to stdout.
...
.exit:
  mov $60, %rax        /* syscall for exit (Linux specific) */
